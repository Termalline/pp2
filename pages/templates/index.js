...
const filteredPosts = blogs.filter(
  ...
  blog.explanation.toLowerCase().includes(searchQuery.toLowerCase()) ||
...
return (
  ...
  <div className="flex items-center justify-between mb-8">
    <h1 className="text-3xl font-bold text-fuchsia-500">Code Templates</h1>
   {userId && <button
    ...
    <input
      ...
      placeholder="Search by title, explanation, tags, or author..."
      className="w-full p-3 border border-gray-300 rounded-lg mb-6 focus:outline-none focus:ring-2 focus:ring-blue-400 text-violet-700 bg-teal-100"
    />
    ...
    {paginatedPosts.length > 0 ? (
      paginatedPosts.map((post) => (  
          ...
          <Link key={blogs.id} href={`/templates/${post.id}`} passHref>
        <div className="border border-gray-300 rounded-lg shadow-lg p-6 bg-green-100 mb-6 cursor-pointer hover:shadow-xl transition-shadow">
          ...
          <p className="text-gray-700 mb-4">{post.explanation.slice(0, 300)}...</p>
